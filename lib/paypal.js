...
export async function verifyPayPalWebhook(headers, rawBody) {
    ...
    // 🔍 DEBUGGING LOGS - THIS WILL REVEAL THE ISSUE
    console.log("🔍 --- DEBUGGING VERIFICATION ---");
    console.log("👉 ENV Webhook ID:", PAYPAL_WEBHOOK_ID); 
    console.log("👉 ID Length:", PAYPAL_WEBHOOK_ID ? PAYPAL_WEBHOOK_ID.length : '0');
    console.log("👉 Auth Algo:", headers.get('paypal-auth-algo'));\
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.log("❌ PAYPAL REJECTED:", response.status);
            console.log("❌ REASON:", errorText); // <--- THIS IS THE KEY
            return false;
        }
